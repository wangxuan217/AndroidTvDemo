...
public class BorderEffect implements BorderView.Effect
{
    ...
    protected long mDurationTraslate = 150;
            ...
            try
            {
                                ...
                                absListView.addOnLayoutChangeListener(new View.OnLayoutChangeListener()
                                {
                                        ...
                                        int oldLeft, int oldTop, int oldRight, int oldBottom)
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                factorX =
                                                    (Math.abs(rect.left - rect.right) - firstView.getMeasuredWidth())
                                                        / 2 - 1;
                                                factorY =
                                                    (Math.abs(rect.top - rect.bottom) - firstView.getMeasuredHeight())
                                                        / 2;
                                            }
                ...
                else if (oldFocus instanceof AbsListView && newFocus instanceof AbsListView)
                {
                    if (attacheViews.indexOf(oldFocus) >= 0 && attacheViews.indexOf(newFocus) >= 0)
                    {
                        ...
                        MyOnItemSelectedListener oldOn =
                            (MyOnItemSelectedListener)onItemSelectedListenerList.get(oldFocus);
                        MyOnItemSelectedListener newOn =
                            (MyOnItemSelectedListener)onItemSelectedListenerList.get(newFocus);
    ...
    protected List<Animator> getMoveAnimator(View newFocus, int factorX, int factorY)
    {
        ...
        try
        {
            ...
            final ObjectAnimator scaleAnimator = ObjectAnimator.ofPropertyValuesHolder(mTarget,
                valuesWithdHolder,
                valuesHeightHolder,
                valuesYHolder,
                valuesXHolder);
            ...
            if (oldFocus != null && newFocus != null)
            {
                if (oldFocus.getParent() != newFocus.getParent())
                {
                    if ((attacheViews.indexOf(newFocus.getParent()) < 0)
                        || (attacheViews.indexOf(oldFocus.getParent()) < 0
                            && attacheViews.indexOf(newFocus.getParent()) > 0))
                    {
                        mTarget.setVisibility(View.INVISIBLE);
                        AnimatorSet animatorSet = new AnimatorSet();
        ...
        try
        {
            ...
            else if (attachView instanceof AbsListView)
            {
                ...
                final AdapterView.OnItemSelectedListener onItemSelectedListener =
                    absListView.getOnItemSelectedListener();
